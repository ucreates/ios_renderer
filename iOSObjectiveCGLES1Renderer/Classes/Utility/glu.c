...
void GLAPIENTRY gluOrtho2D(GLdouble left, GLdouble right, GLdouble bottom, GLdouble top) { glOrthof(left, right, bottom, top, -1.0, 1.0); }
...
void GLAPIENTRY gluPickMatrix(GLdouble x, GLdouble y, GLdouble width, GLdouble height, const GLint viewport[4]) {
    GLfloat m[16];
    GLfloat sx, sy;
    GLfloat tx, ty;
    sx = viewport[2] / width;
 ...
 * New in GLU 1.3
 */
GLboolean GLAPIENTRY gluCheckExtension(const char *extName, const GLubyte *extString) {
    assert(extName);
    assert(extString);
    {
        ...
        while (1) {
            ...
            if ((c == start || c[-1] == ' ') && (c[len] == ' ' || c[len] == 0)) return GL_TRUE;
            start = c + len;
        }
    }
}
