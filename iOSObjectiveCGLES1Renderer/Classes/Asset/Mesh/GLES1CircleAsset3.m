...
// ======================================================================
#import "GLES1CircleAsset3.h"
#import "GLES1Angle.h"
#import "GLES1Renderer.h"
@interface GLES1CircleAsset3 ()
@end
@implementation GLES1CircleAsset3
...
- (void)create {
    ...
    colors[colorsLength - 4] = self->_color.r;
    colors[colorsLength - 3] = self->_color.g;
    colors[colorsLength - 2] = self->_color.b;
    colors[colorsLength - 1] = self->_color.a;
...
- (void)create:(NSString*)texturePath textureUnit:(GLenum)textureUnit {
    ...
    int colorsLength = vertexCount * kRGBA;
    int colorsMemsize = colorsLength * sizeof(GLfloat);
    int uvMemsize = verticesLength * sizeof(GLfloat);
    GLfloat* vertices = (GLfloat*)malloc(vertexMemsize);
    GLfloat* colors = (GLfloat*)malloc(colorsMemsize);
    GLfloat* uvs = (GLfloat*)malloc(uvMemsize);
    int didx = 0;
    vertices[verticesLength - 2] = 0.0f;
    vertices[verticesLength - 1] = 0.0f;
    uvs[verticesLength - 2] = curatio;
    uvs[verticesLength - 1] = cvratio;
    for (int i = 2; i < verticesLength; i += 6) {
        GLfloat unitDegree = (360.0 / self->_divideCount);
        GLfloat degree1 = (GLfloat)didx * unitDegree;
        GLfloat degree2 = (GLfloat)(didx + 1) * unitDegree;
        GLfloat radian1 = [GLES1Angle toRadian:degree1];
        ...
        uvs[i + 1] = cvratio;
        uvs[i + 2] = u1 + curatio;
        uvs[i + 3] = -1 * v1 + cvratio;
        uvs[i + 4] = u2 + curatio;
        uvs[i + 5] = -1 * v2 + cvratio;
        didx++;
    }
    colors[colorsLength - 4] = self->_color.r;
    colors[colorsLength - 3] = self->_color.g;
    colors[colorsLength - 2] = self->_color.b;
    colors[colorsLength - 1] = self->_color.a;
    for (int i = 4; i < colorsLength; i += 12) {
        colors[i] = self->_color.r;
        colors[i + 1] = self->_color.g;
        colors[i + 2] = self->_color.b;
        colors[i + 3] = self->_color.a;
    ...
    [self->_vertex setVerticies:vertices verticiesCount:verticesLength];
    [self->_vertex setColors:colors vertexColorsCount:colorsLength];
    [self->_vertex setUVs:uvs uvsCount:verticesLength];
    return;
}
- (GLenum)renderMode {
